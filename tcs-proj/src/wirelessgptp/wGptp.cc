...
simsignal_t wGptp::localTimeSignal = cComponent::registerSignal("localTime");
simsignal_t wGptp::timeDifferenceSignal = cComponent::registerSignal("timeDifference");
simsignal_t wGptp::rateRatioSignal = cComponent::registerSignal("rateRatio");
simsignal_t wGptp::peerDelaySignal = cComponent::registerSignal("peerDelay");
...
const MacAddress wGptp::GPTP_MULTICAST_ADDRESS("01:80:C2:00:00:0E");
...
wGptp::~wGptp()
{
    ...
    cancelAndDeleteClockEvent(hopChannelMsg);
}

void wGptp::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LINK_LAYER) {
        ...
        if (*str) {
            ...
            nic->subscribe(packetReceivedFromPeerSignal, this);
            nic->subscribe(packetSentToUpperSignal, this);
        }
        else
            if (gptpNodeType != MASTER_NODE)
        ...
        channelNumber = par("initialChannelNumber");
        EV_INFO << "Initial Channel Number: " << channelNumber << endl;
        channelHopInterval = par("channelHopInterval");
        EV_INFO << "Channel Hop Interval: " << channelHopInterval << endl;

        std::string channelHopType = par("channelHopType");
        if(channelHopType == "incremental")
        {
            EV_INFO << "Channel Hop Type: INCREMENTAL_CHANNEL_HOP" << endl;
            hopChannelMsg = new ClockEvent("hopChannelMsg", INCREMENTAL_CHANNEL_HOP);
            scheduleClockEventAfter(par("channelHopInitialOffset"), hopChannelMsg);
        } else {
            EV_INFO << "ChannelHopType: NONE" << endl;
        }
    }
}

void wGptp::hopChannel() {
    channelNumber = (channelNumber + 1) % 12;
    EV_INFO << "Hopping to channel: " << channelNumber << endl;
    if (slavePortId != -1) {
        auto nic = interfaceTable->getInterfaceById(slavePortId);
        EV_INFO << "Hopping channel for slave port " << nic << endl;
        cModule* radioModule = nic->getSubmodule("radio");
        if (radioModule) {
            auto radio = dynamic_cast<physicallayer::Ieee80211Radio*>(radioModule);
            if (radio) {
                radio->setChannelNumber(channelNumber);
            } else {
                EV_ERROR << "Unable to cast slave port " << nic << " to Ieee80211Radio" << endl;
            }
        }
    }
    for (auto id: masterPortIds) {
        auto nic = interfaceTable->getInterfaceById(id);
        EV_INFO << "Hopping channel for master port " << nic << endl;
        cModule* radioModule = nic->getSubmodule("radio");
        if (radioModule) {
            auto radio = dynamic_cast<physicallayer::Ieee80211Radio*>(radioModule);
            if (radio) {
                ...
                EV_ERROR << "Unable to cast master port " << nic << " to Ieee80211Radio" << endl;
            }
        }
    }
}

void wGptp::handleSelfMessage(cMessage *msg)
{
    ...
    switch(kind) {
        ...
        // slaveport:
            ...
            break;
        case INCREMENTAL_CHANNEL_HOP:
            hopChannel();
            scheduleClockEventAfter(channelHopInterval, hopChannelMsg);
            break;
        default:
...
void wGptp::handleMessage(cMessage *msg)
{
...
void wGptp::sendPacketToNIC(Packet *packet, int portId)
{
...
void wGptp::sendFollowUp(int portId, const GptpSync *sync, clocktime_t preciseOriginTimestamp)
{
...
void wGptp::sendPdelayResp(GptpReqAnswerEvent* req)
{
    ...
    auto requestReceiptTimeStamp = req->getIngressTimestamp();
    gptp->setRequestReceiptTimestamp(requestReceiptTimeStamp);
    packet->insertAtFront(gptp);
    sendPacketToNIC(packet, portId);
    // The sendPdelayRespFollowUp(portId) called by receiveSignal(), when GptpPdelayResp sent
}

void wGptp::sendPdelayRespFollowUp(int portId, const GptpPdelayResp* resp)
{
    auto packet = new Packet("GptpPdelayRespFollowUp");
    packet->addTag<MacAddressReq>()->setDestAddress(GPTP_MULTICAST_ADDRESS);
    auto gptp = makeShared<GptpPdelayRespFollowUp>();
    auto responseOriginTimestamp = clock->getClockTime();
    gptp->setDomainNumber(domainNumber);
    gptp->setResponseOriginTimestamp(responseOriginTimestamp);
...
void wGptp::processFollowUp(Packet *packet, const GptpFollowUp* gptp)
{
...
void wGptp::processPdelayReq(Packet *packet, const GptpPdelayReq* gptp)
{
...
void wGptp::processPdelayResp(Packet *packet, const GptpPdelayResp* gptp)
{
    // verify IDs
...
void wGptp::processPdelayRespFollowUp(Packet *packet, const GptpPdelayRespFollowUp* gptp)
{
    if (!rcvdPdelayResp) {
...
void wGptp::receiveSignal(cComponent *source, simsignal_t signal, cObject *obj, cObject *details)
{
    ...
    if (signal == receptionEndedSignal) {
        ...
        if (packet) {
            ...
            if (*protocol == Protocol::ieee80211OfdmPhy) {
                const auto& wPhyHeader = packet->peekAtFront<physicallayer::Ieee80211OfdmPhyHeader>();
                const auto& wMacHeader = packet->peekAt<ieee80211::Ieee80211MacHeader>(wPhyHeader->getChunkLength());
                if (wMacHeader->getType() == ieee80211::Ieee80211FrameType::ST_DATA) {
                    const auto& llcSnapHeader = packet->peekAt<Ieee8022LlcSnapHeader>(wPhyHeader->getChunkLength() + wMacHeader->getChunkLength());
                    if (llcSnapHeader->getOui() == 0) {
                        if (llcSnapHeader->getProtocolId() == ETHERTYPE_GPTP) {
                            const auto& gptp = packet->peekAt<GptpBase>(wPhyHeader->getChunkLength() + wMacHeader->getChunkLength() + llcSnapHeader->getChunkLength());
    ...
    else if (signal == packetReceivedFromPeerSignal) {
        //EV_INFO << "ASDF" << endl;
    }
    else if (signal == packetSentToUpperSignal) {
        auto radio = dynamic_cast<inet::physicallayer::Ieee80211Radio*>(source);
        if (radio) {
            auto packet = check_and_cast_nullable<Packet *>(obj);
            if (packet) {
                auto protocol = packet->getTag<PacketProtocolTag>()->getProtocol();
                if (*protocol == Protocol::ieee80211Mac) {
                    const auto& chunk = packet->peekAtFront();
                    if (chunk && dynamicPtrCast<const ieee80211::Ieee80211DataHeader>(chunk) != nullptr){
                        const auto& wMacHeader = packet->peekAtFront<ieee80211::Ieee80211DataHeader>();
                        auto frameType = wMacHeader->getType();
                        if (frameType == ieee80211::Ieee80211FrameType::ST_DATA) {
                            auto wMacHeaderLength = wMacHeader->getChunkLength();
                            const auto& llcSnapHeaderChunk = packet->peekDataAt(wMacHeaderLength);
                            if (llcSnapHeaderChunk) {
                                std::string chunkTypeName = llcSnapHeaderChunk->getClassName();
                                if (dynamicPtrCast<const Ieee8022LlcSnapHeader>(llcSnapHeaderChunk) != nullptr) {
                                    const auto& llcSnapHeader = packet->peekDataAt<Ieee8022LlcSnapHeader>(wMacHeaderLength);
                                    if (llcSnapHeader->getOui() == 0) {
                                        if (llcSnapHeader->getProtocolId() == ETHERTYPE_GPTP) {
                                            auto llcSnapHeaderLength = llcSnapHeader->getChunkLength();
                                            const auto& gptp = packet->peekDataAt<GptpBase>(wMacHeaderLength + llcSnapHeaderLength);
                                            if (gptp->getDomainNumber() == domainNumber)
                                            {
                                                // INGRESS TIMESTAMP MEASUREMENT PLANE
                                                auto ingressTimestamp = clock->getClockTime();
                                                packet->addTagIfAbsent<GptpIngressTimeInd>()->setArrivalClockTime(ingressTimestamp);
                                            }
                                        }
                                    }
                                }
                            }
                            /* This snippet did not work:
                                auto wMacHeaderLength = wMacHeader->getChunkLength();
                                const auto& chunk = packet->peekDataAt(wMacHeaderLength);
                                std::string chunkTypeName = chunk->getClassName();
                                if (chunkTypeName == "inet::Ieee8022LlcSnapHeader")
                                {
                                    auto chunkLength = chunk->getChunkLength();
                                    auto llcSnapHeader = packet->peekAt<Ieee8022LlcSnapHeader>(wMacHeaderLength, chunkLength, Chunk::PF_ALLOW_REINTERPRETATION);
                                }
                            */
                            /* This snippet gave that the chunkTypeName == "Ieee8022LlcSnapHeader":
                                const auto& chunk = packet->peekDataAt(wMacHeader->getChunkLength());
                                std::string chunkTypeName = chunk->getClassName();
                                EV_INFO << "The chunk type at the offset is: " << chunkTypeName << endl;
                            */
                            /* These did not work:
                                const auto& llcSnapHeader = packet->peekAt<Ieee8022LlcSnapHeader>(wMacHeader->getChunkLength());
                                const auto& ethMacHeader = packet->peekAt<EthernetMacHeader>(wMacHeader->getChunkLength());
                                auto encPacket = packet->getEncapsulatedPacket();
                            */
                        }
                    }
                }
        ...
        auto transmission = dynamic_cast<inet::physicallayer::ITransmission*>(obj);
                            ...
                            if (gptp->getDomainNumber() == domainNumber) {
                                ...
                                switch (gptp->getMessageType()) {
                                    ...
                                    case GPTPTYPE_SYNC: {
                                        ...
                                        auto originTime = clock->getClockTime();
                                        sendFollowUp(portId, gptpSync.get(), originTime);
                                        break;
                                    }
